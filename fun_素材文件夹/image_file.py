...
class ImageFile:
    ...
    def preview_image_path(self, material_path: Path, preview_path: Path) -> Path:
        """素材图转换到预览图."""
        return Path(self.path.as_posix().replace(material_path.as_posix(), preview_path.as_posix()))

    @threads(5)
    def fun_复制素材图到预览图(self, material_path: Path, preview_path: Path) -> None:
        """复制素材图到预览图."""
        preview_path = self.preview_image_path(material_path=material_path, preview_path=preview_path)
        preview_path.parent.mkdir(parents=True, exist_ok=True)

        if preview_path.exists() is not True:
            MaterialType.log(f"复制素材图到预览图{self.path.as_posix()}\t->\t{preview_path.as_posix()}")

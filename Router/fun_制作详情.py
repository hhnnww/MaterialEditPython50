...
from MaterialEdit import fun_制作数据图, fun_制作详情栏目标题, ImageEdit, fun_保存图片, fun_制作详情, fun_遍历图片, fun_裁剪图片, fun_清空桌面上传文件夹图片
...
def fun_make_material_product_image(item: MakeProductImageRequestModel):
    ...
    data_pil = fun_制作数据图(
        [
            ...
            ("* 非图片", "本店素材并非图片，为专业设计师使用的设计源文件，非设计师请勿购买。"),
            ("* 样机须知", "本店样机均为白膜样机，不包含样机内贴图，购买前请知晓。"),
        ]
    )
    data_im = ImageEdit.fun_图片竖向拼接(
        [data_header, data_pil], spacing=0, align_item="center", background_color=(255, 255, 255, 255)
    )
    ...
    if item.has_effect_image == 1 and Path(item.effect_image_path).exists() is True:
        image_list = fun_遍历图片(folder=item.effect_image_path, used_image_number=0, image_sort=True)
        ...
        if len(image_list) > 0:
            ...
            data_im = ImageEdit.fun_图片竖向拼接(
                [header_pil, effect_image], spacing=0, align_item="center", background_color=(255, 255, 255, 255)
            )
    ...
    if item.has_preview_image == 1 and Path(item.preview_image_path).exists() is True:
        ...
        if len(image_list) > 0:
            ...
            data_im = ImageEdit.fun_图片竖向拼接(
                [header_pil, preview_image], spacing=0, align_item="center", background_color=(255, 255, 255, 255)
            )

...
class MakeCalenderPic:
    ...
    def fun_判断横竖版本(png_list: list[Image.Image]):
        average_ratio = sum([obj.width / obj.height for obj in png_list]) / len(png_list)
    ...
    def fun_获取文件夹所有图片(sub_path: Path) -> list[Image.Image]:
        png_file_list = [Image.open(obj.as_posix()) for obj in fun_遍历指定文件(folder=sub_path.as_posix(), suffix=[".png"])]
    ...
    def fun_制作单排图片(self, pil_list: list[Image.Image]) -> Image.Image:
        ...
        return ImageEdit.fun_图片横向拼接(pil_list, self.gutter, "center", (255, 255, 255, 255))
    ...
    def fun_制作日历图片(self, comb_pil_list: list[list[Image.Image]]) -> Image.Image:
        pil = ImageEdit.fun_图片竖向拼接(
            [self.fun_制作单排图片(obj) for obj in comb_pil_list], self.gutter, "center", (255, 255, 255, 255)
        )
        pil = ImageEdit.fun_图片扩大粘贴(
            pil, self.width, pil.height + (self.gutter * 2), "center", "center", (255, 255, 255, 255)
        )
        return pil

    def main(self):
        for in_path in tqdm(list(self.material_path.iterdir()), ncols=100, desc="制作日历图片\t"):

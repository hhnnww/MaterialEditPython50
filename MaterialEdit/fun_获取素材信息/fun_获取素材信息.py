...
def fun_获取素材信息(root_path: str, used_image: int, image_sort: bool):
    ...
    preview_image_list = [
        dict(path=obj) for obj in fun_遍历图片(folder=preview_image_path, image_sort=image_sort, used_image_number=0)
    ]
    ...
    effect_image_list = [
        dict(path=obj) for obj in fun_遍历图片(folder=effect_image_path, image_sort=image_sort, used_image_number=0)
    ]
